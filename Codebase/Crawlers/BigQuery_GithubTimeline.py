...
############## Loggers ##########
fileConfig(SCRIPT_LOCATION + "logging.conf")
debug_log = logging.getLogger("root")
error_log = logging.getLogger("performance")
...
def saveResult(query_response):
    ...
    """
    debug_log.info("Saving the query result")
    result_row_final2 = []
    new_repo_list = []
    for row in query_response['rows']:
	    ...
	    error_log.exception("Error while saving the row")
	    continue
    debug_log.info("returning the parsed data to CSV")
...
def main():
    ...
    """
    debug_log.info("Started Pulling the information")
    debug_log.info("Started Pulling the information")
    ...
    while num_days>0:
        ...
        try:
            ...
            for i in range(0, num_iter):
                ...
                query_data = {"query":'select type,payload,repo.name,actor.login,FORMAT_UTC_USEC(created_at) as created,org.id,org.login \
                               from [githubarchive:day.events_%(tablename)s] where TIMESTAMP(created_at) >= TIMESTAMP("%(start_date)s") \
                ...
                try:
                    ...
                    error_log.exception("Error in returning the query_response since the response might be huge")
                    error_log.info("try again by changing the value of STEP_SIZE_HOURS to one of these(4,3,2,1)")
                ...
                if ('rows' in query_response):
                    ...
                    while ('rows' in query_response and currentRow < query_response['totalRows']):
                        try:
                            ...
                            error_log.exception("Error in getting subsequent pages")
                            continue
                        if ('rows' in query_response):
                            df_temp, repo_info_temp = saveResult(query_response)
                            df = df.append(df_temp)
            ...
            error_log.exception("=============== Connection refused ===================")
            return False
    	...
    	if filename + ".csv" not in glob(DATA_LOCATION+"timeline_files/*csv"):
    	    write_file = open(DATA_LOCATION + "timeline_files/" + filename +".csv", "a")
    	    df.to_csv(write_file, index=False)
        else:
            write_file = open(DATA_LOCATION + "timeline_files/" + file_name +".csv", "a")
